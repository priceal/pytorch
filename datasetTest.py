...
if __name__ == "__main__":
    ...
    batchSize = 200  # only use if numBatches = 0
    numberEpochs = 200
    reportCycle = 10
    ...
    for i in range(numberEpochs):
        ...
        for j, batch in enumerate(dataloader):
            ...
            lossTerms = - \
                batch[1]*torch.log(prediction)-(1.0-batch[1]) * \
            ...
            if i % reportCycle == 0:
                ...
                print(f"{i:<10} {j:<10} {loss.item():<10} {lossTest.item():<10}")
                plt.plot([i], [loss.item()], '.k')
                plt.plot([i], [lossTest.item()], '.r')
